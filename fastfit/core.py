...
conv_dict = {np.dtype('int8'): torch.LongTensor, 
...
def V_(x, requires_grad=False, volatile=False):
    r"""equivalent to create_variable, which creates a pytorch tensor.
    """
...
def to_gpu(x, *args, **kwargs):
    r"""
    puts pytorch variable to gpu, 
    if cuda is available and USE_GPU is set to true."""
...
def split_by_idxs(seq, idxs):
    r"""
    A generator that returns sequence pieces, seperated by 
    indexes specified in idxs.
    """
    last = 0
    for idx in idxs:
        if not (-len(seq) <= idx < len(seq)):
          raise KeyError(f'Index {idx} is out-of-bounds')
        yield seq[last:idx]
        last = idx
    yield seq[last:]
...
def partition(a, sz): 
    r"""splits iterables a in equal parts of size sz.
    """
...
class BasicModel():
    def __init__(self,model,name='basic'): 
        self.model = model
        self.name = name
...
def load(fn): 
    r"""Utility function that loads model, function, etc as pickle.
    """
    return pickle.load(open(fn, 'rb'))
def load2(fn):
    r"""Utility funciton allowing model piclking across Python2 and Python3.
    """
