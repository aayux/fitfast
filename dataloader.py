...
class DataLoader(object):
    ...
    Arguments:
        ...
        pred_pad (bool, optional): ??? DOCUMENTATION REQUIRED ???
        half (bool, optional): ??? DOCUMENTATION REQUIRED ???
        transpose (bool, optional): ??? DOCUMENTATION REQUIRED ???
        transpose_y (bool,  optional): ??? DOCUMENTATION REQUIRED ???

    """
    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None, 
                 batch_sampler=None, pad_idx=0, num_workers=None, 
    ...
    def _jag_stack(self, batches):
        ...
        batch = np.zeros((len(batches), ml), 
                          dtype=batches[0].dtype) + self.pad_idx
    ...
    def _get_batch(self, idxs):
        bacth = self._np_collate([self.dataset[idx] for idx in idxs])
